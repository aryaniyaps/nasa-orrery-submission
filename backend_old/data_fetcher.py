    ...
    return [
        {
            ...
            "position": [body["semimajorAxis"], body["periapsis"], body["apoapsis"]],
        }
        for body in planetary_data
        if body["isPlanet"]
    ]
...
def fetch_comet_data(api_key):
    ...
    return [
        {"name": comet["name"], "position": [comet["a"], comet["e"], comet["i"]]}
        for comet in comets_data
    ]
...
def fetch_all_celestial_objects(api_key):
    ...
    combined_data = {"planets": planets, "comets": comets, "asteroids": asteroids}
